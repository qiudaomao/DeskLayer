//
// prelude.js — pure-JS view builders injected into every plugin context.
//
// This is the canonical copy, shared by every platform port; the mac app
// bundles it verbatim (scripts/check-docs-sync.sh enforces the match).
// Any host must inject this file into a fresh plugin context before
// evaluating the plugin source, and must implement the __dl_resetActions /
// __dl_invokeAction contract described below.
//
// Building a tree costs zero JS→host bridge calls: builders make plain
// objects and chainable modifier methods just append {name, args}. Methods
// are invisible to JSON.stringify, so the tree serializes as clean data
// matching the ViewNode wire format. Aliases: Section→VStack, Paragraph→Text.
//
...
// native view fires, the host calls __dl_invokeAction(id, payloadJSON). The
// table is rebuilt each render (reset via __dl_resetActions before render()),
// so ids always match the tree just produced. Input only reaches
// floating-window items — the wallpaper layer ignores mouse events.
//
(function (global) {
    ...
    // them. The host calls this just before invoking render().
    global.__dl_resetActions = function () {
        global.__dl_actions = {};
        nextActionId = 1;
    };

    // Called by the host when a native Button/tap/text-change fires. The
